...
void CBase64Coder::fnSetData(const char *pData,int iLen)
{
	m_pData = new char[iLen+1];
	memset(m_pData,0,iLen+1);
	memcpy(m_pData,pData,iLen);
}

char* CBase64Coder::fnGetData()
{
	return m_pData;
}

bool CBase64Coder::fnEncodeData()
{
	char * pData = base64Encode(m_pData,(int)strlen(m_pData));
	delete[] m_pData;
	m_pData = pData;
	return true;
}

bool CBase64Coder::fnDecodeData()
{
	unsigned int uiLong = 0;
	char * pData = (char*)base64Decode(m_pData,uiLong);
	delete[] m_pData;
	m_pData = pData;
	*(m_pData+uiLong) = 0;
    return true;
